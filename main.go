...
func registerActions(loader *bt.Loader) {
	...
	loader.RegisterAction("enter_phase2", func(ctx *bt.Context) bt.Status {
		...
		if phase != 2 {
			ctx.BB.Set("_phase", 2)
			ctx.Log("⚔ [阶段] 炎魔将军 - Phase2 狂暴!\n")
			ctx.Log("  \"你们这些蝼蚁...让我认真起来!\"\n")
		}
		return bt.Success
	})
	...
	loader.RegisterActionWithReset("play_stun", func(ctx *bt.Context) bt.Status {
		...
		if ticks < 3 {
			ctx.Log("★ 炎魔将军被眩晕中... (%d/3)\n", ticks)
			return bt.Running
		}
		ctx.Log("★ 炎魔将军眩晕结束, 恢复行动!\n")
	...
	loader.RegisterAction("counter_attack", func(ctx *bt.Context) bt.Status {
		ctx.Log("★ 炎魔将军反击一刀!\n")
		return bt.Success
	})
	...
	loader.RegisterAction("chase", func(ctx *bt.Context) bt.Status {
		ctx.Log("→ 炎魔将军向玩家冲去!\n")
		return bt.Success
	})
	...
	loader.RegisterActionWithReset("cast_ultimate", func(ctx *bt.Context) bt.Status {
		...
		if ticks < 4 {
			ctx.Log("→ 炎魔将军蓄力【炎灭天地】... (%d/4) [Running]\n", ticks)
			return bt.Running
		}
		ctx.BB.Set("_ult_ticks", 0)
		ctx.BB.Set("ultimate_ready", false)
		ctx.Log("→ 炎魔将军释放【炎灭天地】!!! 全屏毁灭!\n")
		return bt.Success
	}, func(ctx *bt.Context) {
		ticks, _ := bt.Get[int](ctx.BB, "_ult_ticks")
		if ticks > 0 {
			ctx.Log("  !! 终极技能蓄力被打断! 技能进入冷却!\n")
			ctx.BB.Set("_ult_ticks", 0)
			ctx.BB.Set("ultimate_ready", false)
		}
	})
	loader.RegisterAction("flee", func(ctx *bt.Context) bt.Status {
		ctx.Log("→ 炎魔将军向后撤退!\n")
		return bt.Success
	})
	loader.RegisterAction("heal", func(ctx *bt.Context) bt.Status {
		ctx.Log("→ 炎魔将军吞噬火焰恢复生命!\n")
...
func main() {
	...
	ticks := []Tick{
		{desc: "玩家进入副本, Boss 巡逻", setup: func(bb *bt.Blackboard) {
			for _, k := range []string{"hp_percent", "_combo", "_phase", "_fb_ticks", "_ult_ticks", "_stun_ticks"} {
				bb.Set(k, 0)
			}
			bb.Set("hp_percent", 100)
			for _, k := range []string{"stunned", "should_dodge", "player_detected", "player_in_range",
				"fireball_ready", "fire_storm_ready", "heal_ready", "ultimate_ready", "minions_summoned"} {
				bb.Set(k, false)
			}
		}},
	...
	for i, tick := range ticks {
		...
		status := exec.Tick()
		fmt.Printf("[结果] %s (running=%v)\n", status, exec.IsRunning())
	}
